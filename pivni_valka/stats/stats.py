...
def save_daily_stats(date: datetime.date, user_name: str, count: int):
    exists = db.query_one('SELECT 1 FROM pivni_valka WHERE `date` = ? and user = ?;', (date, user_name))
...
def _get_days() -> int:
    return db.query_one('SELECT COUNT(`date`) FROM (SELECT DISTINCT `date` FROM pivni_valka)')[0]


def get_total_unique_beers() -> dict[str, int]:
    return {value[0]: value[1] for value in db.query_all(
